...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct 10 19:17:10 2020
...
parser.add_argument('--input_fsdata_train', , type=str, help='fsdata')
...
class MlpNetC1(nn.Module):
    def __init__(self, trial, layer_sizes, input_size):
        super(MlpNetC1, self).__init__()
        layers = []
        layer_sizes = [input_size] + layer_sizes
        activation=get_activation(trial,'act1')
        for l_id in range(len(layer_sizes) - 1):
            if l_id == len(layer_sizes) - 2:
                layers.append(nn.Sequential(
                    nn.BatchNorm1d(num_features=int(layer_sizes[l_id]), affine=False),
                    nn.Linear(int(layer_sizes[l_id]), int(layer_sizes[l_id + 1])),
                ))
            else:
                layers.append(nn.Sequential(
                    nn.Linear(int(layer_sizes[l_id]), int(layer_sizes[l_id + 1])),
                    activation(),
                    nn.BatchNorm1d(num_features=int(layer_sizes[l_id + 1]), affine=False),
                ))
...
class MlpNetC2(nn.Module):
    def __init__(self, trial, layer_sizes, input_size):
        ...
        layers = []
        layer_sizes = [input_size] + layer_sizes
        activation=get_activation(trial,'act2')
        for l_id in range(len(layer_sizes) - 1):
            if l_id == len(layer_sizes) - 2:
                layers.append(nn.Sequential(
                    nn.BatchNorm1d(num_features=int(layer_sizes[l_id]), affine=False),
...
class DeepCCA(nn.Module):
    def __init__(self, trial, layer_sizes1, layer_sizes2, input_size1, input_size2, outdim_size, use_all_singular_values, device=torch.device('cpu')):
        super(DeepCCA, self).__init__()
        self.model1 = MlpNetC1(trial,layer_sizes1, input_size1).double()
        self.model2 = MlpNetC2(trial,layer_sizes2, input_size2).double()
...
def get_activation(trial,name):
    activation_names = ['ReLU', 'ELU', 'Sigmoid']
    activation_name = trial.suggest_categorical(name, activation_names)
...
def objective(trial):
    layer_num1=trial.suggest_int('num_layer1',3,7)
    layer_num2=trial.suggest_int('num_layer2',3,7)
    l_size11=trial.suggest_discrete_uniform('layer_size1',200,1000,200)
    
    layer_sizes1 = []
    for itr in range(layer_num1):
        layer_sizes1.append(l_size11)
    layer_sizes1.append(outdim_size)
    
    
    l_size12=trial.suggest_discrete_uniform('layer_size2',200,1000,200)
    layer_sizes2 = []
    for itr in range(layer_num2):
        layer_sizes2.append(l_size12)
    layer_sizes2.append(outdim_size)
    
    learning_rate=trial.suggest_loguniform('learning_rate', 1e-6,1e-3)
    batch_size=trial.suggest_discrete_uniform('batch_size',500,1500,300)
    reg_par=trial.suggest_loguniform('reg_par', 1e-8,1e-2)
    
    model = DeepCCA(trial,layer_sizes1, layer_sizes2, input_shape1,
    ...
    # train
    solver = Solver(model, l_cca, outdim_size, epoch_num, int(batch_size),
                learning_rate, reg_par, device=device)
    solver.fit(train1, train2, val1, val2)
    
    # test
    loss, outputs, NN_outputs = solver.test(torch.cat([val1], dim=0), torch.cat([val2], dim=0), apply_linear_cca)
...
shuffled_id=np.random.permutation(len(nmf_score_train))
train1_npy,val1_npy=np.split(nmf_score_train[shuffled_id],[int(0.8*len(nmf_score_train))],)
train2_npy,val2_npy=np.split(lda_score_train[shuffled_id],[int(0.8*len(lda_score_train))],)

train1, train2 = make_tensor((train1_npy, train2_npy))
val1, val2 = make_tensor((val1_npy,val2_npy))
#test1, test2 = make_tensor((nmf_score_test,lda_score_test))
np.save(SCRATCH+'/DCCA/optuna_train_val',shuffled_id)
# %% tuning
...
pd.Series(best_params).to_csv(SCRATCH+'/DCCA/best_params.csv')
